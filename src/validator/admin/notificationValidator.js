...
exports.create = catchAsync(async (req, res, next) => {
  let rules = {
    title: "required",
    body: "required",
    type: "required|in:basic,standard",
    castingType: "required|in:now,scheduled",
    scheduledAt: "required_if:castingType,scheduled",
  };
...
exports.update = catchAsync(async (req, res, next) => {
  ...
  let { id } = req.body;
  let check = await Notification.findOne({ _id: id });
  if (!check) {
